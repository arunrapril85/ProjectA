...
import com.mobily.followup.info.BMSOrganizationTypeInfo;
import com.mobily.followup.info.CustomerProfileInfo;
import com.mobily.followup.service.balanceinquiry.BalanceInquiryService;
...
import com.mobily.followup.service.mnp.MNPUnreachedHoldTimer;
...
import com.mobily.followup.service.pool.BufferingFixTimer;
...
public class FollowupServicesMain {
...
//			CCProperties.p_Settings = Util.readProperty("config/dummy");
		...
		///BY ASHRAF	
			...
			CCProperties.allMSISDNAssetCategories = CreditLimitUtil.loadMSISDNAssetCategories();
			...
			CCProperties.allHUCCPackageTypes = facade.getHUCCPackageTypes();
//			CCProperties.allBMSOrgTypes = BMSFacade.getInstance().getAllBMSOrgTypes();
			...
			BMSPortalFacade bmsPortalfacade = BMSPortalFacade.getInstance();
			CCProperties.allBMSServiceOrderTypes = bmsPortalfacade.getAllBMSServiceOrderTypes();
...
//			try {
//				System.out.println("Going to send welcome message");
//				MQHelper mqhelper 				= MQHelper.instance();
//				mqhelper.sendToMQ("Welcom", "MOBILY.FUNC.FUS.FMS.ALARM.REQUEST");
//				System.out.println("welcome message successfully sent");
//			}catch(Exception e){
//				e.printStackTrace();
//			}
...
//			log.debug("Starting the WelcomeCallPoolBufferingTimer is done");	
			...
			log.info("Going to Start the BufferingFixTimer ");
			BufferingFixTimer bufferingFixTimer = new BufferingFixTimer();
			log.debug("Starting the BufferingFixTimer is done");
...
/////////////////////////////////////////////			
			...
			log.info("Going to Start SiebelOutletsSynchronizer ");
			//SiebelOutletsSynchronizer siebelOutletsSynchronizer = new SiebelOutletsSynchronizer();
			//siebelOutletsSynchronizer.execute();
			...
			}catch( Exception supExp)
			{
				supExp.printStackTrace();
			}
	...
	private static void startScheduledTasks()
    {
		try {
			  ...
			  String ccMNPUnreachedHoldTimerExpression 	  = (String)CacheManager.getCachedValue("com.mobily.bms.mnp.unreached.hold.timer.job");
...
*/
		      ...
		      /// Define the MNP Unreached Hold JOB DETAILS
		      JobDetail mnpUnreachedHoldJobDetail   = new JobDetail( "MNP_Unreached_Hold_Timer_JOB" , Scheduler.DEFAULT_GROUP, MNPUnreachedHoldTimer.class );
		      CronTrigger mnpUnreachedHoldTimerTrigger   = new CronTrigger("MNP_Unreached_Hold_Timer_TRIGGER_NAME" , Scheduler.DEFAULT_GROUP, ccMNPUnreachedHoldTimerExpression );
...
//		      CronTrigger smsBroadcastTimerTrigger   = new CronTrigger("SMS_Broadcast_Timer_TRIGGER_NAME" , Scheduler.DEFAULT_GROUP, ccSMSBroadcastTimerCronExpression );
		      ...
		      log.debug("ReportTimer job is successfully");		      
...
//		      log.debug("BMS Service Order Monitor job is started successfully");
		      ...
		      log.debug("Going to schedule the MNP Unreached Hold job");
		      sched.scheduleJob( mnpUnreachedHoldJobDetail , mnpUnreachedHoldTimerTrigger );
		      log.debug("MNP Unreached Hold job is scheduled successfully");
